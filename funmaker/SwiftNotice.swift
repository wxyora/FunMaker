...
class SwiftNotice: NSObject {
    ...
    static func wait(_ imageNames: Array<UIImage> = Array<UIImage>(), timeInterval: Int = 0) {
        ...
        if imageNames.count > 0 {
            ...
            ai.frame = CGRect(x: 33, y: 12, width: 45, height: 45)
            ai.startAnimating()
        ...
        let label = UILabel(frame: CGRect(x: 19, y: 63, width: 80, height: 12))
    ...
    static func showNoticeWithText(_ type: NoticeType,text: String, autoClear: Bool, autoClearTime: Int) {
        ...
        checkmarkView.frame = CGRect(x: 33, y: 12, width: 45, height: 45)
        mainView.addSubview(checkmarkView)
        
        //let label = UILabel(frame: CGRectMake(0, 60, 90, 16))
        let label = UILabel(frame: CGRect(x: 16, y: 63, width: 80, height: 12))
